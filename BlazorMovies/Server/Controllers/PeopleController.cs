    ...
    public class PeopleController : ControllerBase
    {
        ...
        public async Task<ActionResult<DetailsPersonDTO>> Get(int id)
        {
            var person = await context.People.Where(x => x.Id == id)
                .Include(x => x.MoviesActors).ThenInclude(x => x.Movie).FirstOrDefaultAsync();

            if (person == null) { return NotFound(); }

            person.MoviesActors = person.MoviesActors.OrderBy(x => x.Movie.ReleaseDate).ToList();
            var model = new DetailsPersonDTO();
            model.Person = person;
            model.Movies = person.MoviesActors.Select(x => x.Movie).ToList();
